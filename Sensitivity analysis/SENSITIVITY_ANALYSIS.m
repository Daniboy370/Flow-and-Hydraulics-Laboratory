...
%% ****************************** PURPOSE **************************** % 
% **** The Script takes ready made set-up of plumes from previous RUN 
% **** (with locations and times). After declaring new desired variables, 
% **** it plots graphs based on the SAME DATA SET.
...
% ***********************  Process definition *********************** %
dt = 0.5;                                               % Step Duration / Sampling Period [sec]
    ...
    for t=1:total_steps
        ...
        if ( t_lin(pl_i).data(t) > Pl_init_loc(pl_i).t )
            ...
            if ( gen_plume_only_once(pl_i).data == 1 )
                ...
                % *********** Putting Plume on the same Location ********** %                
                [M_pl(pl_i).transport.x, M_pl(pl_i).transport.y, M_pl(pl_i).transport.z] = generate_single_plume( N_particles, h, W);
                % ****** Positioning each PLUME on its specific Location ****** %
                M_pl(pl_i).transport = put_initial_loc_par ( M_pl(pl_i).transport, Pl_init_loc(pl_i).x, Pl_init_loc(pl_i).y );
        ...
        if( s_ti(pl_i).ix <= save_f(pl_i).length && t_lin(pl_i).data(t) >= save_f(pl_i).frames( s_ti(pl_i).ix ) )           % Threshold for indices
            ...
            M_pl( pl_i ).time_frame( s_ti(pl_i).ix ).x = M_pl( pl_i ).transport.x;
            M_pl( pl_i ).time_frame( s_ti(pl_i).ix ).y = M_pl( pl_i ).transport.y;
            M_pl( pl_i ).time_frame( s_ti(pl_i).ix ).z = M_pl( pl_i ).transport.z;
...
if (need_sensor_calc == 1)                                       % *** save details to FILE "flag"
    ...
    for pl_i=1:num_plm                                           % ****** summation of all structs' arrays
        ...
        sensor_each_analysis(:,2) = sensor_t(pl_i).conc(:,1);             % *** SUPERPOSITION sum of Columns of EACH PLUME;
        Sensor_FILE_analysis(:,2) = Sensor_FILE_analysis(:,2) + sensor_each_analysis(:,2);
        if ( need_sensor_show == 1)
            show_sensor_t( sensor_each_analysis, 'k--', show_legend);     % ****** Display EACH PLUME seperately (black)
        end
    end
